...
- 추가 수정(사용자 확인): FORK_FEATURES.md에 명시된 "호버 중 단어가 거터 폭만큼 밀려나 바가 드러남" 효과가 초기 구현에서 빠졌던 것을 복원. `.blyrics-line-main > *`(하이라이트 오버레이 + 본문 텍스트, 번역/로마자/후리가나 행은 별도 줄이라 영향 없음)에 `translate`를 걸고, `isInSeekGutter`/`placeGutterBar`는 호버 중 측정한 단어 rect가 이미 밀려나 있는 만큼 되돌려서 판정/바 위치가 밀림에 따라 같이 틀어지지 않게 했다.
- 버그 수정(사용자 확인): 바/슬라이드가 전혀 안 보이던 원인은 CSS 명시도(specificity) 문제였다. "평상시" 규칙(`.blyrics-container:not(...) > .blyrics--line:not(...) .blyrics-line-main::before`)이 클래스 3개+속성 2개로 "호버" 규칙(`.blyrics--line.blyrics--seek-hover .blyrics-line-main::before`, 클래스 3개)보다 더 구체적이어서, 호버 클래스가 JS로 붙어도 평상시 값(`opacity:0`, `translate:0`)이 계속 이겼다. 컨테이너/간주 가드를 `:where()`로 감싸 명시도에서 제외하고, 평상시·호버 규칙의 명시도를 동일하게 맞춰 소스 순서(호버가 나중에 옴)로 결정되게 했다.
- 추가 수정(사용자 확인): 바가 마우스를 따라 움직이던 원인은 `placeGutterBar`를 매 `mousemove`마다(슬라이드 트랜지션 도중에도) 다시 측정해서였다 - 트랜지션 중간의 단어 rect를 "이미 다 밀렸다"는 고정 보정치로 되돌리다 보니 220ms 동안 위치가 계속 바뀌었고, 마침 그 사이 마우스도 움직이니 "마우스 따라 움직인다"로 보였다. 줄에 처음 호버 진입할 때(호버 클래스 붙이기 직전, 즉 아직 슬라이드 시작 전 rest 위치)만 한 번 측정해 고정하도록 변경. 거터 폭도 1.5rem→2.25rem로 키움(히트존과 시각 슬라이드가 같은 값을 공유하므로 둘 다 커짐).
- 추가 수정(사용자 확인): "바가 오른쪽으로 계속 이동", "진입하자마자 이탈" 둘 다 같은 원인 - 호버 중에도 `isInSeekGutter`가 매 mousemove마다 슬라이드 트랜지션 도중의 실제 DOM을 다시 재고, "이미 다 밀렸다"는 고정치로 보정하고 있었다. 트랜지션이 끝나지 않은 상태에서 그 보정이 어긋나면 막 진입한 순간 판정 영역이 포인터 밖으로 튀어 즉시 이탈하고, 이탈→슬라이드 원복→재진입이 반복되며 바도 같이 흔들렸다. 아예 설계를 바꿔, 줄에 처음 진입할 때(아직 슬라이드 전) 텍스트 위치를 한 번만 재서 캐시하고, 호버가 유지되는 동안은 그 캐시값만으로 판정한다(DOM을 다시 읽지 않음) - 클릭 캡처 리스너도 현재 호버 중인 줄이면 이 캐시를 그대로 쓰고, 아닌 줄이면(호버 없이 바로 클릭한 경우 등) 새로 측정한다.
- 튜닝/버그 수정(사용자 요청): 거터 폭 2.25rem→1.6875rem(0.75배), 바 두께 0.09em→0.27em(3배). 왼쪽 정렬(가장 흔한) 줄에서 바가 안 보이던 원인은, 오른쪽 정렬 줄은 바를 텍스트 오른쪽 끝에 딱 붙여 얇은 두께만큼만 배치한 반면 왼쪽 정렬 줄은 `textLeft - gutter`(거터 전체 폭만큼 왼쪽으로) 배치해서였다. 코어의 줄 인라인 패딩이 0.25em뿐이라 왼쪽으로 그만큼 벌어질 여백이 없어 조상 요소에 잘렸다. 히트존 계산에 이미 있던 클램프(`textLeft - rect.left`로 제한)를 바 배치에도 그대로 적용해, 여백이 부족하면 바가 클릭 가능한 실제 폭까지만 좁혀지도록 했다(잘리지 않음).
- 추가 튜닝/버그 수정(사용자 요청): 거터 폭 1.6875rem→1.5em, 바 두께 0.27em→0.15em(단위를 rem→em으로 변경, 줄 자체의 font-size 기준 - 활성/비활성 줄은 `transform: scale()`만 다르고 font-size는 항상 같아서 em이 줄마다 흔들리지 않는다. JS도 루트 대신 줄의 `getComputedStyle().fontSize`를 읽도록 맞춤). 오른쪽 정렬 줄도 잘려 보이던 것은, 왼쪽 정렬만 여백 부족 시 판정 폭만큼 좁히는 클램프가 있고 오른쪽 정렬은 텍스트 오른쪽 끝에 바 두께만큼 무조건 붙이기만 해서 여백이 없으면 오른쪽으로 넘쳤기 때문 - 오른쪽에도 동일한 클램프를 적용해 항상 줄 상자 안에 들어오게 했다.
- 좌표계 버그 수정 + 튜닝(사용자 확인): 바(`.blyrics-line-main::before`)의 `left`는 line-main 자신의 박스를 기준으로 계산되는데, JS는 지금까지 바깥 `.blyrics--line`의 rect를 기준으로 삼아 왔다. 두 박스는 코어의 인라인 패딩(0.25em)만큼 어긋나 있고, 그 오차가 클램프와 맞물리면 여백이 적은 쪽(특히 오른쪽 정렬)에서 바가 line-main 박스 밖으로 나가 안 보였다. 모든 계산의 기준을 `.blyrics-line-main` 자신의 rect로 통일. 가운데 정렬(`v1000`)은 텍스트가 줄 상자 가운데에 떠 있어 좌우 여백이 항상 넉넉해서 클램프가 거의 걸리지 않는 반면, 왼쪽 정렬은 텍스트가 거의 붙어 있어 클램프가 강하게 걸린다 - 그래서 두 정렬의 "뜨는 범위"가 원래도 다르게 계산된다(구조적인 차이). 거터 폭은 1.5em→0.5em(1/3)로 축소.
