...
import {
  ...
  markFuriganaSwapped,
  removeFurigana,
} from "@modules/lyrics/furiganaDom";
...
export async function annotateFuriganaFromRomaji(
  ...
  if (line.lyricElement.querySelector(`.${FURIGANA_CLASS}`)) return true;
  return attachFuriganaRuns(line.lyricElement.ownerDocument, line, surface, runs) > 0;
}
 ...
 */
export async function annotateFurigana(line: LineData, fullText: string): Promise<boolean> {
  ...
  if (line.lyricElement.querySelector(`.${FURIGANA_CLASS}`)) return true;
  return attachFuriganaRuns(line.lyricElement.ownerDocument, line, fullText, runs) > 0;
}
...
export function applyLlmFurigana(
  ...
  if (existing.length === runs.length && existing.every((reading, i) => reading === runs[i].reading)) {
    return false;
  }

  removeFurigana(line);
  attachFuriganaRuns(line.lyricElement.ownerDocument, line, fullText, runs);
  markFuriganaSwapped(line);
  return true;
}
